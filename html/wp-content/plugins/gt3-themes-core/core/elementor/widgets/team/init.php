	...
	class GT3_Core_Elementor_Widget_Team extends \ElementorModal\Widgets\GT3_Core_Widget_Base {
		...
		public function render_team_item( $posts_per_line, $single_member = false, $grid_gap = '', $link_post = '', $custom_item_height = '', $settings = false ) {
			...
			if ( strlen( $wp_get_attachment_url ) ) {
				...
				$gt3_featured_image_url = aq_resize( $wp_get_attachment_url, $gt3_featured_image_size, $gt3_featured_image_size * ( 1167 / 1140 ), true, true, true );
			...
			if ( ! $single_member ) {
                ...
                <div class="item_wrapper">
                    <div class="item">
                        ' . ( ! empty( $featured_image ) ? '<div class="team_img featured_img">' . ( $link_post == 'yes' ? '<a href="' . get_permalink( get_the_ID() ) . '">' . $featured_image . '</a>' : $featured_image ) . '</div>' : '' ) .
                            ...
                            <div class="team_title">
                                <div class="team_title_wrapper">
                                    <h3 class="team_title__text">' . ( $link_post == 'yes' ? '<a href="' . get_permalink( get_the_ID() ) . '">' . get_the_title() . '</a>' : get_the_title() ) . '</h3>' .
				            ( ! empty( $positions_str[0] ) ? '<div class="team-positions">' . $positions_str[0] . '</div>' : '' ) . '</div>' . '
                            </div>' .
				            ( ! empty( $short_desc ) ? '<div class="team_info"><div class="member-short-desc">' . $short_desc . '</div></div>' : '' ) .
				            ( ! empty( $signature ) ? '<div class="team_signature"><img  src="' . $signature . '" alt="' . get_the_title() . '" /></div>' : '' ) .
				            ( !empty($view_more) ? $view_more : '' ) .
				            '</div>
                    </div>
                </div>
            </li>';

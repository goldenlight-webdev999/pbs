...
if ( $settings['image_size'] && is_array( $settings['image_size'] ) ) {
	$image_style = 'style="';
	$image_style .= $settings['image_size']['width'] ? 'width: '.$settings['image_size']['width'].'px;' : '';
	$image_style .= $settings['image_size']['height'] ? 'height: '.$settings['image_size']['height'].'px;' : '';
	$image_style .= '"';
}
	...
	switch($settings['icon_image']) {
		...
		case 'image':
			if(isset($settings['image']['id']) && (bool) $settings['image']['id']) {
				...
				if($image) {
					$icon = '<div class="icon_container" '.$image_style.'><img src="'.aq_resize($image['0'], $settings['image_size']['width']*2, $settings['image_size']['height']*2, true, true, true).'" alt="" /></div>';
				} else {

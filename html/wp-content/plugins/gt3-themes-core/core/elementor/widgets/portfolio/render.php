...
if($query->have_posts()) {
	...
	<div <?php $widget->print_render_attribute_string('wrapper') ?>>
			...
			<div class="isotope-filter">
				...
				if (count($query_raw['taxonomy']) > 1) {
		            ...
		            foreach ($cats as $cat) {
			            ...
			            echo '<a href="'.esc_url($permalink).'" data-filter=".'.esc_attr($cat->slug).'"'.($_REQUEST[$portfolio_query_arg] == $cat->term_id ? ' class="active"' : '').'>';
		...
		?>
		<div class="isotope_wrapper items_list gt3_clear<?php echo esc_attr($portfolio_classes); ?>">
			<?php

...
if(!class_exists('ElementorModal\Widgets\GT3_Core_Elementor_Widget_Portfolio')) {
	class GT3_Core_Elementor_Widget_Portfolio extends \ElementorModal\Widgets\GT3_Core_Widget_Base {
		...
		public function get_title(){
			return esc_html__('Portfolio', 'gt3_themes_core');
		}
			...
			if(!$image_id) {
				...
				if ($settings) {
					...
					if (!empty($image_array) && is_array($image_array)) {
						$image_src = array();
				        $image_src[0] = !empty($image_array[0]) ? $image_array[0] : wp_get_attachment_url($image_id, 'full');
			...
			if ($settings) {
				...
				if (!empty($image_src[0]) && strlen($image_src[0])) {
		            ...
		            if (function_exists('getSolidColorFromImage')) {
            			...
            			$featured_image .= '<div class="gt3_portfolio_list__image-placeholder'.(!empty($gt3_featured_image_class) ? ' gt3_lazyload__placeholder' : '').'" style="padding-bottom:'.(100*$ration).'%;'.(!$lazyload ? 'margin-bottom:-'.(100*$ration).'%;' : '' ).'background-color:#'.$mainColor.';"></div>';
		            }		            

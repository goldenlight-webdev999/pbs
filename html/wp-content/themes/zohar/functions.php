...
if (!function_exists('gt3_get_theme_option')) {
    function gt3_get_theme_option($optionname, $defaultValue = null){
        $gt3_options = get_option("zohar_gt3_options");
...
if(!function_exists('gt3_option')) {
    function gt3_option($name) {
        if (  class_exists( 'Redux' ) ) {
            $theme_options = get_option( 'zohar' );
            if (empty($theme_options)) {
                $theme_options = get_option( 'zohar_default_options' );
            }
            return isset($theme_options[$name]) ? $theme_options[$name] : null;
        }else{
            $default_option = get_option( 'zohar_default_options' );
        ...
        <div id="comment-<?php comment_ID(); ?>" class="stand_comment">
            <div class="thiscommentbody">
                ...
                <div class="comment_info">
                    <div class="comment_author_says"><?php printf('%s', get_comment_author_link()) ?> <span><?php esc_html_e('says:', 'zohar'); ?></span></div>
                    <div class="listing_meta">
                        <span><?php printf('%1$s', get_comment_date()) ?></span>
                        <?php edit_comment_link('<span>('.esc_html__('Edit', 'zohar').')</span>', '  ', '') ?>
                    </div>
                </div>
                <div class="comment_content">
                    <?php if ($comment->comment_approved == '0') : ?>
                        <p><?php esc_html_e('Your comment is awaiting moderation.', 'zohar'); ?></p>
                    <?php endif; ?>
                    <?php comment_text() ?>
                </div>
                <?php
                ...
                comment_reply_link(array_merge($args, array('depth' => $depth, 'reply_text' => '' . (($icon_post_comments)) . esc_html__('Reply', 'zohar'), 'max_depth' => $max_depth_comment)))
                ?>
            </div>
        </div>
        <?php
...
if (!function_exists('gt3_page_title')) {
    function gt3_page_title(){
	    ...
	    if ( class_exists( 'WooCommerce' ) && is_product() ) {
            ...
            $title = single_term_title("", false).esc_html__(' Tag', 'zohar');
        }elseif (is_date()) {
            $title = get_the_time('F Y');
        }elseif(is_author()){
            $title = esc_html__('Author:', 'zohar') . " " . esc_html(get_the_author());
        }elseif (is_search()) {
            $title = esc_html__('Search', 'zohar');
        }elseif (is_404()) {
            $title = esc_html__('404', 'zohar');
        }elseif (is_archive()) {
            $title = esc_html__('Archive','zohar');
            ...
            if (!empty($post)) {
                ...
                $title = esc_html__('No Posts','zohar');
            }
        }
...
function gt3_the_breadcrumb(){
    ...
    if(is_front_page() && !is_home()) {
        ...
        if (is_category()) {
            ...
            echo wp_kses_post($before) . esc_html__('Archive','zohar').' "' . single_cat_title('', false) . '"' . wp_kses_post($after);
            ...
            echo  wp_kses_post($before) . esc_html__('Search for','zohar').' "' . esc_html(get_search_query()) . '"' . wp_kses_post($after);
            ...
            echo  wp_kses_post($before) . esc_html__('Tag','zohar').' "' . single_tag_title('', false) . '"' . wp_kses_post($after);

        } elseif (is_author()) {
            global $author;
            $userdata = get_userdata($author);
            echo  wp_kses_post($before) . esc_html__('Author','zohar').' ' . esc_html($userdata->display_name) . wp_kses_post($after);

        } elseif (is_404()) {
            echo  wp_kses_post($before) . esc_html__('Error 404','zohar') . wp_kses_post($after);
                    ...
                    echo '<div class="gt3_linear-loading-center">';
                        echo '<div class="gt3_linear-loading-center-absolute">';
                            if (!empty($preloader_logo_url)) {
                                echo '<img style="width:'.esc_attr((int)$preloader_logo_width/2).'px;height: auto;" src="'.esc_url($preloader_logo_url).'" alt="'.esc_attr__('preloader', 'zohar').'">';
                            }
...
if (!function_exists('gt3_get_page_title')) {
    function gt3_get_page_title($id) {
        ...
        $gt3_page_title = is_home() && is_front_page() ? esc_html__('Blog', 'zohar') : gt3_page_title();
        ...
        if ($page_title_conditional == 'yes' && !empty($gt3_page_title)) {
            ...
            echo '<div class="gt3-page-title_wrapper">';
                echo "<div class='gt3-page-title". (!empty($page_title_classes) ? esc_attr($page_title_classes) : '' ) ."'".( !empty($page_title_styles) ? ' style="'.esc_attr($page_title_styles).'"' : '').">";
                        ...
                        echo "<div class='container'>";
                            echo "<div class='gt3-page-title__content'>";
                                if (is_single() && get_post_type() == 'post') {
                                    echo "<div class='gt3_page_title_cats'>";
                                        the_category(' ');
                                    echo "</div>";
                                }
                                ...
                                if (is_single() && get_post_type() == 'post') {
                                    if (get_comments_number(get_the_ID()) == 1) {
                                        $comments_text = ' ' . esc_html__('comment', 'zohar') . '';
                                    } else {
                                        $comments_text = ' ' . esc_html__('comments', 'zohar') . '';
                                    }
                                    $post   = get_post(get_the_ID());
                                    echo "
                                        <div class='page_title_meta'>
                                            <span class='post_date'>". esc_html(get_the_time(get_option( 'date_format' ))) ."</span>
                                            <span class='post_author'>". esc_html__('by', 'zohar'). " <a href='". esc_url(get_author_posts_url($post->post_author)) ."'>". esc_html(get_the_author_meta('display_name', $post->post_author))."</a></span>";
                    ...
                    if (is_single() && (get_post_type() == 'post' || get_post_type() == 'portfolio')) {
                        $prev_post = get_adjacent_post(false, '', true);
                        $next_post = get_adjacent_post(false, '', false);
                        if ($prev_post) {
                            $post_url_prev = get_permalink($prev_post->ID);
                            echo "<a href='" . esc_url($post_url_prev) . "' title='" . esc_attr($prev_post->post_title) . "' class='page_title_post_link prev_link'><span>" . esc_html__('Prev', 'zohar') . "</span></a>";
                        }
                        if ($next_post) {
                            $post_url_next = get_permalink($next_post->ID);
                            echo "<a href='" . esc_url($post_url_next) . "' title='" . esc_attr($next_post->post_title) . "' class='page_title_post_link next_link'><span>" . esc_html__('Next', 'zohar') . "</span></a>";
                        }
                    }
...
if (!function_exists('gt3_get_logo')) {
    function gt3_get_logo() {
        ...
        if (!empty($header_logo_src)) {
            $logo .= '<img class="default_logo" src="'.esc_url($header_logo_src).'" alt="'.esc_attr__('logo', 'zohar').'"'.$logo_height_style.'>';
        }else{
        ...
        if (!empty($logo_sticky_src)) {
            $logo .= '<img class="sticky_logo" src="'.esc_url($logo_sticky_src).'" alt="'.esc_attr__('logo', 'zohar').'"'.$sticky_logo_height_style.'>';
        }
        if (!empty($logo_tablet_src)) {
            $logo .= '<img class="tablet_logo" src="'.esc_url($logo_tablet_src).'" alt="'.esc_attr__('logo', 'zohar').'">';
        }
        if (!empty($logo_mobile_src)) {
            $logo .= '<img class="mobile_logo" src="'.esc_url($logo_mobile_src).'" alt="'.esc_attr__('logo', 'zohar').'">';
        }
        $logo .= "</a>";
        $logo .= "</div>";
        return $logo;
                            ...
                            if (($is_nextend_facebook || $is_nextend_google || $is_nextend_twitter) && get_option('woocommerce_enable_myaccount_registration')=='yes') {
                                ...
                                if ($is_nextend_facebook) {
                                    echo "<div class='gt3_module_button button_alignment_inline'>";
                                        ...
                                        echo '<span>'.esc_html__( 'Login with Facebook', 'zohar' ).'</span>';
                                        echo "</a>";
                                    echo "</div>";
                                }
                                if ($is_nextend_google) {
                                    echo "<div class='gt3_module_button button_alignment_inline'>";
                                        ...
                                        echo '<span>'.esc_html__( 'Login with Google', 'zohar' ).'</span>';
                                        echo "</a>";
                                    echo "</div>";
                                }
                                if ($is_nextend_twitter) {
                                    echo "<div class='gt3_module_button button_alignment_inline'>";
                                        echo "<a href='" . esc_url(wp_login_url() . "?loginSocial=twitter&redirect=" . get_permalink()) . "' class='button_size_normal gt3_twitter_login' title='".esc_attr__( 'Login with Twitter', 'zohar' )."' data-plugin='nsl' data-action='connect' data-redirect='current' data-provider='twitter' data-popupwidth='475' data-popupheight='175'>";
                                        echo '<i class="fa fa-twitter" aria-hidden="true"></i>';
                                        echo '<span>'.esc_html__( 'Login with Twitter', 'zohar' ).'</span>';
                                        echo "</a>";
                                    echo "</div>";
                                }
                                echo "</div>";
                                        ...
                                        switch ($key) {
	                                        ...
	                                        case 'login':
		                                        ...
		                                        if ( is_user_logged_in() ) {
			                                        ...
			                                        echo '<p>' . esc_html__( 'login / register', 'zohar' ) . '</p></div>';
		                                        }
		                                        echo '</div>';
		                                        break;
                                            case 'cart':
                                                if ( class_exists('WooCommerce') ) {
                                                    ...
                                                    ob_start();
                                                    ?>
                                                    <a class="woo_icon" href="<?php echo wc_get_cart_url(); ?>" title="<?php esc_attr_e( 'View your shopping cart', 'zohar' ); ?>">
...
$style_formats = array(
array(
'title' => esc_html__('Font Weight', 'zohar'), 'items' => array(
array('title' => esc_html__('Default', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => 'inherit')),
array('title' => esc_html__('Lightest (100)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '100')),
array('title' => esc_html__('Lighter (200)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '200')),
array('title' => esc_html__('Light (300)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '300')),
array('title' => esc_html__('Normal (400)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '400')),
array('title' => esc_html__('Medium (500)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '500')),
array('title' => esc_html__('Semi-Bold (600)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '600')),
array('title' => esc_html__('Bold (700)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '700')),
array('title' => esc_html__('Bolder (800)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '800')),
array('title' => esc_html__('Extra Bold (900)', 'zohar'), 'inline' => 'span', 'classes' => 'gt3_font-weight', 'styles' => array('font-weight' => '900'))
),
),
);

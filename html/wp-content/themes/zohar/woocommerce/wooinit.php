...
function gt3_add_label_outofstock() {
	...
	if ( ! ( $product->is_in_stock() ) ) {
		echo '<div class="gt3-product-outofstock"><span class="gt3-product-outofstock__inner">' . esc_html__( 'Out Of Stock', 'zohar' ) . '</span></div>';
	}
}
...
function gt3_track_product_view() {
	$gt3_recently_viewed = gt3_option( 'woocommerce_recently_viewed' );
	if ( ! is_singular( 'product' ) || ! (bool) $gt3_recently_viewed ) {
		return;
	}
...
function gt3_woo_header_pagination() {
	...
	$products_infinite_scroll = gt3_option( 'products_infinite_scroll' );
	if ( ( $woocommerce_pagination == 'top' || $woocommerce_pagination == 'top_bottom' ) && $products_infinite_scroll !== 'always' ) {
...
function gt3_size_guide() {
	...
	if ( ! empty( $size_guide_url ) ) {
		echo '<div class="gt3_block_size_popup"><a href="#" class="image_size_popup_button theme_icon-home-repair">' . esc_html__( 'Size Guide', 'zohar' ) . '</a></div><!-- gt3_block_size_popup -->';
	}
}
		...
		<?php if ( $previous_product ) : ?>
            <li>
                <a href="<?php echo esc_url( $previous_product->get_permalink() ); ?>" rel="prev">
					...
					echo '</span>';
					echo '<span class="nav_text">' . esc_html__( 'PREV', 'zohar' ) . '</span>';
		...
		<?php if ( $next_product ) : ?>
            <li>
                <a href="<?php echo esc_url( $next_product->get_permalink() ); ?>" rel="next">
					...
					echo '</span>';
					echo '<span class="nav_text">' . esc_html__( 'NEXT', 'zohar' ) . '</span>';
...
function woo_add_custom_general_field() {
	...
	woocommerce_wp_checkbox( array(
		'id'          => '_checkbox_hot',
		'label'       => esc_html__( 'Hot Product', 'zohar' ),
		'description' => esc_html__( 'Check for Hot Product', 'zohar' )
	) );
	woocommerce_wp_checkbox( array(
		'id'          => '_checkbox_new',
		'label'       => esc_html__( 'New Product', 'zohar' ),
		'description' => esc_html__( 'Check for New Product', 'zohar' )
	) );
	echo '</div>';
}
...
function gt3_hot_new_product() {
	...
	if ( 'yes' == $is_hot ) {
		echo '<span class="onsale hot-product">' . esc_html__( 'Hot', 'zohar' ) . '</span>';
	}

	$is_new = get_post_meta( $product->get_id(), '_checkbox_new', true );
	if ( 'yes' == $is_new ) {
		echo '<span class="onsale new-product">' . esc_html__( 'New', 'zohar' ) . '</span>';
	}
}
...
function gt3_variable_choose_an_option_rename( $args ) {
	...
	if ( is_object( $attr ) ) {
		...
		$fix = esc_html__( 'an option', 'zohar' );
	}
	$args['show_option_none'] = esc_html__( 'Choose ', 'zohar' ) . $fix;
...
if ( ! function_exists( 'gt3_search_category' ) ) {
	function gt3_search_category() {
		...
		if ( empty( $search_cat_hide_empty ) ) {
			$search_cat_hide_empty = '0';
		} // 0 means false
		if ( empty( $search_cat_exclude_child ) ) {
			$search_cat_exclude_child = '0';
		} // 0 means false
		if ( empty( $search_cat_exclude_mobile ) ) {
			$search_cat_exclude_mobile = 'hide';
		}

		$gt3_product_cat = ! empty( $_GET['gt3_product_cat'] ) ? $_GET['gt3_product_cat'] : 0;

		$settings = array(
			'show_option_all'  => $search_cat_text_dropdown,
			'show_option_none' => '',
			'orderby'          => 'name',
			'order'            => 'ASC',
			'show_last_update' => 0,
		...
		$form = '<div id="gt3_cat_search" class="search_cat_mobile_' . esc_attr( $search_cat_exclude_mobile ) . '"> 
            <form action="' . get_permalink( wc_get_page_id( "shop" ) ) . '" method="GET" role="search">
                ...
                <input placeholder="' . esc_attr__( 'Enter your keyworld...', 'zohar' ) . '" type="text" value="' . get_search_query() . '" name="s" id="s" onblur="if (this.value == \'\') {this.value = \'' . esc_attr( $search_cat_text_search ) . '\';}"  onfocus="if (this.value == \'' . esc_attr( $search_cat_text_search ) . '\') {this.value = \'\';}" />
...
function gt3_woocommerce_product_gallery_trigger() {
	echo '<div class="woocommerce-product-gallery__trigger">' . esc_html__( 'Fullscreen', 'zohar' ) . '</div>';
}
...
add_filter( 'body_class', function ( $classes ) {
	$products_infinite_scroll = gt3_option( 'products_infinite_scroll' );
	if ( ! empty( $products_infinite_scroll ) && $products_infinite_scroll !== 'none' ) {
		$classes[] = 'infinite_scroll-' . esc_attr( $products_infinite_scroll );
	}

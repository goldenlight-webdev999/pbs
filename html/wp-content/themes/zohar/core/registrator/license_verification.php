...
function gt3_get_product_name (){
    $product = 'Zohar – Business Consulting Elementor WordPress Theme';
    return $product;
}
...
function gt3_account_activation($code = '',$email = ''){
    ...
    if (!empty($email)) {
        ...
        if ( $response_code != 200 || is_wp_error( $version_info ) ) {
            $errore_return = new WP_Error( 'registration-error-connection-error', esc_html__( 'Registration Connection error', 'zohar' ) );
            return json_encode($errore_return);
        }
    }else{
        $errore_return = new WP_Error( 'registration-error-invalid-email', esc_html__( 'Please provide a valid email address.', 'zohar' ) );
        return json_encode($errore_return);
    }
...
function gt3_registration_notice() {
  ...
  <div class="notice notice-error" style="padding: 5px 25px 15px 75px;position: relative;">
    ...
    <p><?php esc_html_e( 'Purchase Validation! Please activate your theme.', 'zohar' ); ?></p>
     <div><a class="button button-primary" href="admin.php?page=<?php echo is_child_theme() == true ? 'Zohar-Child' : 'Zohar' ?>&tab=<?php echo class_exists('WooCommerce') ? '20' : '17'; ?>"><?php esc_html_e( 'Register Now', 'zohar' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a target="_blank" class="button button-primary" href="http://themeforest.net/cart/add_items?item_ids=23194664&ref=gt3themes"><?php esc_html_e( 'Buy Theme', 'zohar' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a style="vertical-align: bottom;margin-left: 10px;line-height: 1.6;" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'account_registration_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss this notice', 'zohar' ); ?></a></div>
  </div>
  <?php
  }
}
  ...
  ?>
    <div class="notice notice-error is-dismissible" style="background-color: #d54e21; color: #ffffff; border-radius: 4px; padding: 10px 25px 10px 75px;position: relative;border-left: none;">
        ...
        if (!empty($support_time_left['expired']) && $support_time_left['expired'] == true) {
            esc_html_e( 'Your support package for this theme expired', 'zohar' ); ?><?php echo " ( ".$support_time_left['time_to_left']." ".esc_html__( 'ago', 'zohar' )." ).";
        }else{
            esc_html_e( 'Your support package for this theme is about to expire', 'zohar' ); ?><?php echo " ( ".$support_time_left['time_to_left']." ".esc_html__( 'left', 'zohar' )." ).";
        }     
        ?></p>
        <div style="margin-bottom: 10px;"><a class="button button-primary" target="_blank" href="<?php echo esc_url($theme_link); ?>" style="color: #ffffff;background: #ffffff;border-color: #ffffff;box-shadow: none;text-shadow: none;color: #d54e21;font-weight: 600;margin-top: 5px;"><?php esc_html_e( 'Update Support Package', 'zohar' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a> <a class="button button-primary" href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'notice_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>" style="color: #ffffff;background: #ffffff;border-color: #ffffff;box-shadow: none;text-shadow: none;color: #d54e21;font-weight: 600;margin-top: 5px;"><?php esc_html_e( 'Dismiss this notice', 'zohar' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>
    </div>
  <?php
    }
}
...
function gt3_hide_hotice (){
    if ( isset( $_GET['gt3-hide-notice'] ) && isset( $_GET['_gt3_notice_nonce'] ) ) {
        if ( ! wp_verify_nonce( $_GET['_gt3_notice_nonce'], 'gt3_hide_notices_nonce' ) ) {
            wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'zohar' ) );
        }
...
function gt3_account_notice(){    
  ...
  <div class="notice notice-warning" style="padding: 10px">
    <p><strong><?php esc_html_e( 'Create account and manage your theme purchase codes.', 'zohar' ) ?></strong></p>
    <p><strong><a href="admin.php?page=<?php echo is_child_theme() == true ? 'Zohar-Child' : 'Zohar' ?>&tab=16"><?php esc_html_e( 'Register Now', 'zohar' ); ?></a> |  <a href="<?php echo esc_url( wp_nonce_url( add_query_arg( 'gt3-hide-notice', 'account_notice_srart' ), 'gt3_hide_notices_nonce', '_gt3_notice_nonce' ) ); ?>"><?php esc_html_e( 'Dismiss this notice', 'zohar' ); ?></a></strong></p>
  </div>
  <?php
}
